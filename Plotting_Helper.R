...
OM_labs = c("Base","BH","Hi h","Lo h", "2*R0","M/2 - BH")
for(o in 1:6){
  if(o==1) { OM_Plot=OM_BASE_MexRec } 
  if(o==2) { OM_Plot=OM_BH_MexRec } 
  if(o==3) { OM_Plot=OM_Hih_MexRec } 
  if(o==4) { OM_Plot=OM_Loh_MexRec } 
  if(o==5) { OM_Plot=OM_lnR0_MexRec }
  if(o==6) { OM_Plot=OM_M_BH_MexRec } 
  for(k in 1:(24/4)){
    kh = k*4
    hlist=c(kh, kh-1, kh-2, kh-3)
    plot(years, OM_Plot$HCR_1$SSB_SSBMSY[,1], type='l', col="white", ylim=c(0, 2.5), ylab="")
...
OM_labs = c("Base","BH","Hi h","Lo h", "2*R0","M/2 - BH")
for(o in 1:6){
  if(o==1) { Results=Results_Base_MexRec } 
  if(o==2) { Results=Results_BH_MexRec } 
  if(o==3) { Results=Results_Hih_MexRec } 
  if(o==4) { Results=Results_Loh_MexRec } 
  if(o==5) { Results=Results_lnR0_MexRec }
  if(o==6) { Results=Results_M_BH_MexRec } 
  for(r in seq(1, 24, by=4)){
    vioplot(Results$SSB_SSBMSY_2115[,r], Results$SSB_SSBMSY_2115[,(r+1)], Results$SSB_SSBMSY_2115[,(r+2)], Results$SSB_SSBMSY_2115[,(r+3)], 
            col=c("dimgrey","deepskyblue3","forestgreen","darkorchid"), ylim=c(0, 1.5), names=c(rep("", 4)) )
    abline(h=1)
