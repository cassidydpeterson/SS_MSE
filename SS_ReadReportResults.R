...
GET_MSE_RESULTS <- function(OM = "Base", hlist = 1:24, iters = seq(47, 245, by = 2), years = 1960:2115, base_dir = "") {
  ...
  for (h in hlist) {
    ...
    lst_files <- list.files(path = file.path(base_dir, paste0("OM_", OM), paste0("HCR", h), "StoreResults"))
...
MANIP_RESULTS <- function(OM_Plot, OM_Name, hlist = 1:24, plot = TRUE) {
  ...
  SSB_SSBMSY_2070 <- matrix(nrow = 100, ncol = 24)
  for (h in 1:24) {
    ssb <- t(as.data.frame(rbind(OM_Plot[[paste0("HCR_", h)]]$SSB_SSBMSY["2070", ], "iter" = as.numeric(colnames(OM_Plot[[paste0("HCR_", h)]]$SSB_SSBMSY["2070", ])))))
    xx <- as.data.frame(seq(from = 47, to = 245, by = 2))
    colnames(xx) <- "iter" # DO THIS TO ACCOUNT FOR MISSING ITERATIONS

    SSB_SSBMSY_2070[, h] <- merge(ssb, xx, all.y = T)[, 2]
  }
  SSB_SSBMSY_2070 <- as.data.frame(SSB_SSBMSY_2070)
  row.names(SSB_SSBMSY_2070) <- names(OM_Plot$HCR_1$SSB_SSBMSY)
  names(SSB_SSBMSY_2070) <- paste0("HCR_", 1:24)
  ...
  FM_FMMSY_2070 <- matrix(nrow = 100, ncol = 24)
  for (h in 1:24) {
    fmsy <- t(as.data.frame(rbind(OM_Plot[[paste0("HCR_", h)]]$FM_FMMSY["2070", ], "iter" = as.numeric(colnames(OM_Plot[[paste0("HCR_", h)]]$FM_FMMSY["2070", ])))))
    xx <- as.data.frame(seq(from = 47, to = 245, by = 2))
    colnames(xx) <- "iter" # DO THIS TO ACCOUNT FOR MISSING ITERATIONS

    FM_FMMSY_2070[, h] <- merge(fmsy, xx, all.y = T)[, 2]
  }
  FM_FMMSY_2070 <- as.data.frame(FM_FMMSY_2070)
  row.names(FM_FMMSY_2070) <- names(OM_Plot$HCR_1$FM_FMMSY)
  names(FM_FMMSY_2070) <- paste0("HCR_", 1:24)
  ...
  Com_Catch_2070 <- matrix(nrow = 100, ncol = 24)
  for (h in 1:24) {
    CC <- t(as.data.frame(rbind(apply(OM_Plot[[paste0("HCR_", h)]]$Com_catch[as.character(2066:2070), ], 2, mean), "iter" = as.numeric(names(apply(OM_Plot[[paste0("HCR_", h)]]$Com_catch[as.character(2066:2070), ], 2, mean))))))
    xx <- as.data.frame(seq(from = 47, to = 245, by = 2))
    colnames(xx) <- "iter" # DO THIS TO ACCOUNT FOR MISSING ITERATIONS

    Com_Catch_2070[, h] <- merge(CC, xx, all.y = T)[, 2]
  }
  Com_Catch_2070 <- as.data.frame(Com_Catch_2070)
  row.names(Com_Catch_2070) <- names(OM_Plot$HCR_1$Com_catch)
  names(Com_Catch_2070) <- paste0("HCR_", 1:24)
  ...
  AvgLen_F_2070 <- matrix(nrow = 100, ncol = 24)
  for (h in 1:24) {
    AL <- t(as.data.frame(rbind(OM_Plot[[paste0("HCR_", h)]]$AvgLen_F["2070", ], "iter" = as.numeric(colnames(OM_Plot[[paste0("HCR_", h)]]$AvgLen_F["2070", ])))))
    xx <- as.data.frame(seq(from = 47, to = 245, by = 2))
    colnames(xx) <- "iter" # DO THIS TO ACCOUNT FOR MISSING ITERATIONS

    AvgLen_F_2070[, h] <- merge(AL, xx, all.y = T)[, 2]
  }
  AvgLen_F_2070 <- as.data.frame(AvgLen_F_2070)
  row.names(AvgLen_F_2070) <- names(OM_Plot$HCR_1$AvgLen_F)
  names(AvgLen_F_2070) <- paste0("HCR_", 1:24)


  ### GET Prob overfishing ###

  ProbOF <- matrix(nrow = 100, ncol = 24)
  for (h in 1:24) {
    POF_a <- OM_Plot[[paste0("HCR_", h)]]$FM_FMMSY[as.character(2016:2115), ]
    POF_b <- ifelse(POF_a > 1, 1, 0)
    POF_c <- cbind("POF" = apply(POF_b, 2, sum) / nrow(POF_b), "iter" = as.numeric(colnames(OM_Plot[[paste0("HCR_", h)]]$FM_FMMSY["2115", ])))
    xx <- as.data.frame(seq(from = 47, to = 245, by = 2))
    colnames(xx) <- "iter"
    ProbOF[, which(hlist == h)] <- merge(POF_c, xx, all.y = T)[, 2]
  } # end Flist
  ProbOF <- as.data.frame(ProbOF)
  row.names(ProbOF) <- seq(from = 47, to = 245, by = 2)
  names(ProbOF) <- paste0("HCR_", 1:24)
  ...
  ll[["SSB_SSBMSY_2070"]] <- SSB_SSBMSY_2070

  ll[["FM_FMMSY_2115"]] <- FM_FMMSY_2115

  ll[["FM_FMMSY_2070"]] <- FM_FMMSY_2070
  ...
  ll[["Com_Catch_2070"]] <- Com_Catch_2070

  ll[["AvgLen_F_2115"]] <- AvgLen_F_2115

  ll[["AvgLen_F_2070"]] <- AvgLen_F_2070
  ...
  if (plot == TRUE) {
    ...
    for (k in 1:(24 / 4)) {
      ...
      if (k == 2) {
        mtext(expression("F"["lim"] * "= M"), side = 3, line = 0, cex = 0.8)
      }
      if (k == 3) {
        mtext(expression("F"["lim"] * "= 0.8M"), side = 3, line = 0, cex = 0.8)
      }
      if (k == 4) {
        mtext(expression("F"["lim"] * "= 0.6M"), side = 3, line = 0, cex = 0.8)
      }
      if (k == 5) {
        mtext(expression("F"["lim"] * "= 0.4M"), side = 3, line = 0, cex = 0.8)
      }
      if (k == 6) {
        mtext(expression("F"["lim"] * "= 0.2M"), side = 3, line = 0, cex = 0.8)
      }
      if (k == 6) {
        legend("topright", c(
          expression("a=0, b=B"["MSY"]), expression("a=0, b=0.8B"["MSY"]),
    ...
    data1 <- matrix(c(1.5, 1, 60000, 1, 120, 0, 0, 0, 0, 80), ncol = 5, byrow = T)
    data2 <- matrix(c(1.5, 1, 60000, 1, 120, 0, 0, 0, 0, 80), ncol = 5, byrow = T)
    data3 <- matrix(c(1.5, 1, 60000, 1, 120, 0, 0, 0, 0, 80), ncol = 5, byrow = T)
    data4 <- matrix(c(1.5, 1, 60000, 1, 120, 0, 0, 0, 0, 80), ncol = 5, byrow = T)
    data5 <- matrix(c(1.5, 1, 60000, 1, 120, 0, 0, 0, 0, 80), ncol = 5, byrow = T)
    data6 <- matrix(c(1.5, 1, 60000, 1, 120, 0, 0, 0, 0, 80), ncol = 5, byrow = T)
    for (h in h) {
      if (h2[h] == 1) {
        data1 <- rbind(data1, c(
          apply(OM_Plot[[paste0("HCR_", h)]]$SSB_SSBMSY, 1, median)[length(years)],
          1 - median(ProbOF[, paste0("HCR_", h)]),
          mean(apply(OM_Plot[[paste0("HCR_", h)]]$Com_catch, 2, sum)),
          1 - AAV_HCR[h],
    ...
    colnames(data2) <- c("SSB2115/SSBMSY", "1-POF", "Cum Com Catch", "1-AAV", "Avg Len 2110-2115")
    row.names(data2) <- c("max", "min", "HCR5", "HCR6", "HCR7", "HCR8")
    data2 <- as.data.frame(data2)
    colnames(data3) <- c("SSB2115/SSBMSY", "1-POF", "Cum Com Catch", "1-AAV", "Avg Len 2110-2115")
    row.names(data3) <- c("max", "min", "HCR9", "HCR10", "HCR11", "HCR12")
    data3 <- as.data.frame(data3)
    colnames(data4) <- c("SSB2115/SSBMSY", "1-POF", "Cum Com Catch", "1-AAV", "Avg Len 2110-2115")
    row.names(data4) <- c("max", "min", "HCR13", "HCR14", "HCR15", "HCR16")
    data4 <- as.data.frame(data4)
    colnames(data5) <- c("SSB2115/SSBMSY", "1-POF", "Cum Com Catch", "1-AAV", "Avg Len 2110-2115")
    row.names(data5) <- c("max", "min", "HCR17", "HCR18", "HCR19", "HCR20")
    data5 <- as.data.frame(data5)
    colnames(data6) <- c("SSB2115/SSBMSY", "1-POF", "Cum Com Catch", "1-AAV", "Avg Len 2110-2115")
    ...
    for (d in list(data1, data2, data3, data4, data5, data6)) {
      ...
      if (counter == 1) {
        mtext(expression("F"["lim"] * "= F"["MSY"]), side = 3, line = -2, cex = 0.8)
      }
      if (counter == 2) {
        mtext(expression("F"["lim"] * "= M"), side = 3, line = -2, cex = 0.8)
      }
      if (counter == 3) {
        mtext(expression("F"["lim"] * "= 0.8M"), side = 3, line = -2, cex = 0.8)
      }
      if (counter == 4) {
        mtext(expression("F"["lim"] * "= 0.6M"), side = 3, line = -2, cex = 0.8)
      }
      if (counter == 5) {
        mtext(expression("F"["lim"] * "= 0.4M"), side = 3, line = -2, cex = 0.8)
      }
      if (counter == 6) {
        mtext(expression("F"["lim"] * "= 0.2M"), side = 3, line = -2, cex = 0.8)
      }
      if (counter == 2) {
        legend("bottom", c(
          expression("a=0, b=B"["MSY"]), expression("a=0, b=0.8B"["MSY"]),
    ...
    par(mfrow = c(2, 3), mar = c(1.1, 1.6, 1, 0.3), tcl = -0.1, mgp = c(0.8, 0.1, 0), cex = 1, oma = c(0, 1.5, 2, 0))
    for (r in seq(1, 24, by = 4)) {
      vioplot(SSB_SSBMSY_2070[, r], SSB_SSBMSY_2070[, (r + 1)], SSB_SSBMSY_2070[, (r + 2)], SSB_SSBMSY_2070[, (r + 3)],
    ...
    mtext(OM_Name, outer = TRUE, cex = 1.5, line = 0)
    mtext(expression("SSB"[2070] * "/SSB"["MSY"]), side = 2, outer = TRUE, cex = 1.5, line = -0.3)
    dev.off()
    ...
    par(mfrow = c(2, 3), mar = c(1.1, 1.6, 1, 0.3), tcl = -0.1, mgp = c(0.8, 0.1, 0), cex = 1, oma = c(0, 1.5, 2, 0))
    for (r in seq(1, 24, by = 4)) {
      vioplot(FM_FMMSY_2070[, r], FM_FMMSY_2070[, (r + 1)], FM_FMMSY_2070[, (r + 2)], FM_FMMSY_2070[, (r + 3)],
    ...
    mtext(OM_Name, outer = TRUE, cex = 1.5, line = 0)
    mtext(expression("F"[2070] * "/F"["MSY"]), side = 2, outer = TRUE, cex = 1.5, line = -0.3)
    dev.off()
    ...
    par(mfrow = c(2, 3), mar = c(1.1, 1.6, 1, 0.3), tcl = -0.1, mgp = c(0.8, 0.1, 0), cex = 1, oma = c(0, 1.5, 2, 0))
    for (r in seq(1, 24, by = 4)) {
      vioplot(Com_Catch_2070[, r], Com_Catch_2070[, (r + 1)], Com_Catch_2070[, (r + 2)], Com_Catch_2070[, (r + 3)],
    ...
    mtext(OM_Name, outer = TRUE, cex = 1.5, line = 0)
    mtext(expression("Commercial Catch"[2070]), side = 2, outer = TRUE, cex = 1.5, line = -0.3)
    dev.off()
    ...
    mtext(OM_Name, outer = TRUE, cex = 1.5, line = 0)
    mtext("POF", side = 2, outer = TRUE, cex = 1.5, line = -0.3)
    #####
    dev.off()
  } # END IF PLOT==T
} # END FUNCTION
